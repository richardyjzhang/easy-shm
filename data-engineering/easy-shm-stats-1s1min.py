"""定时聚合脚本 —— 由 crontab 每分钟调用一次。

从 origin_data 库的 origin_data 超级表中，查询所有子表，
对 [now-2min, now-1min) 窗口内的原始数据执行 1s / 1min 两种粒度聚合，
将结果写入 stats_data 库的 stats_data 超级表对应子表中。
"""
...
def main():
    now = datetime.now()
    win_end = now.replace(second=0, microsecond=0) - timedelta(minutes=1)
